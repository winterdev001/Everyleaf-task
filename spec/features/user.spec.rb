...
RSpec.feature "user management function", type: :feature do 
 ...
 scenario "display  users task only" do   
    ...
    User.create!(name: 'dev', email: 'deva@gmail.com', password: 'dev123')
    user=User.first
    @user = User.last
    ...
    @task =   Task.create!(task_name: "holla11", label_name: "hellohello11",start_date: '2019-10-5 1:40:00',status: 'not started', deadline: '2019-10-15 5:40:00' , priority: 'High', user_id: @user.id)
    ...
    if @task.user_id == current_user.id
      expect(page).to have_content 'holla11'
      expect(page).not_to have_content  'holla'
    end
  end
